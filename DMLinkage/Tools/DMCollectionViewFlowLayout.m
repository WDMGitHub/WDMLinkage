...
@implementation DMCollectionViewFlowLayout
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    [noneHeaderSections enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //获取当前section在正常情况下已经脱离屏幕的header结构信息
        UICollectionViewLayoutAttributes *attributes = [self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader atIndexPath:indexPath];
        
        //如果当前分区确实有因为离开屏幕而被系统回收的header
        if (attributes)
        {
            //将该header结构信息重新加入到supperArray中去
    ...
    for (UICollectionViewLayoutAttributes *attributes in superArray)
    {
        ...
        if ([attributes.representedElementKind isEqualToString:UICollectionElementKindSectionHeader])
        {
            ...
            //第一个cell的y值 - 当前header的高度 - 可能存在的secitonInset的top
            ...
            //最后一个cell的y值 + 最后一个cell的高度 + 可能存在的secitonInset的bottom - 当前header的高度
            //当当前section的footer或者下一个section的header接触到当前header底部，计算出的headerMissingY即为有效值
            CGFloat headerMissingY = CGRectGetMaxY(lastItemAttributes.frame) + self.sectionInset.bottom - rect.size.height;
            //给rect的y赋新值，因为在最后消失的临界点要跟随消失，所以取小
            rect.origin.y = MIN(maxY, headerMissingY);
            //给header的结构信息的frame重新赋值
            attributes.frame = rect;
            //如果按照正常情况下，header离开屏幕被系统回收，而header的层次关系又与cell相等，如果不去处理，会出现cell在header上面的情况
